...
from src.db import TelemetryBase


class ChillerTelemetry(TelemetryBase):
    ...
    chiller_unit_id: Mapped[int] = mapped_column(Integer, nullable=False, index=True)
    ...
    # Relationships to the metadata database are not enforced here because telemetry
    # lives in a dedicated historical database.
